...
export default function CustomCheckbox({ ...props }: CustomCheckboxPops) {
  return (
    <label className={styles.label}>
      <input className={styles.input} type='checkbox' checked={props.checked} onChange={props.onChange} />
      <span className={styles.checkbox}>{props.checked ? <EnableIcon /> : <DisableIcon />}</span>
      {props.label}
    </label>
  );
}
